...
CONTAINER_NAME ?= primerevenue/postgres_exporter:latest
...
docker-build:
	docker run -v $(shell pwd):/go/src/github.com/primerevenue/postgres_exporter \
	    -v $(shell pwd):/real_src \
	    -e SHELL_UID=$(shell id -u) -e SHELL_GID=$(shell id -g) \
	    -w /go/src/github.com/primerevenue/postgres_exporter \
		golang:1.8-wheezy \
